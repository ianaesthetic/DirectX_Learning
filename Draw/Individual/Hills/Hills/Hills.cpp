...
void Hills::DrawScene() {
	...
	XMMATRIX localWorld = XMLoadFloat4x4(&mLtoW);
	XMMATRIX worldView = XMLoadFloat4x4(&mWtoV);
	XMMATRIX viewProj = XMLoadFloat4x4(&mVtoH);
	XMMATRIX WVP = localWorld * worldView * viewProj;

	/*XMFLOAT4X4 t;
	XMStoreFloat4x4(&t, WVP);
	...
	*/
	...
	cWVP = mFx->GetVariableByName("gWVP")->AsMatrix();
...
void Hills::BuildGeometryBuffer() {
	...
	for (int i = 0; i <= mGridH; ++i) {
		for (int j = 0; j <= mGridW; ++j) {
			...
			//fout << x << " " << z << " " << y << " " << j << " " << i << " " << Terrain.IndexQuery(j, i) << endl; 
...
void Hills::OnMouseMove(WPARAM btnState, int x, int y) {
	if ((btnState & MK_LBUTTON) != 0) {
		...
		mTheta += dx;
		mPhi -= dy;
		//mPhi = Clamp(0.0f, 0.5f * Pi - 0.001f, mPhi);
	}
